...
object GoogleVsApple {
  ...
  def getTweetsByKeywordsFromTweetSet(tweetSet: TweetSet)(
      keywords: List[String]
  ): TweetSet =
    tweetSet.filter(tweet => keywords.exists(tweet.text.contains(_)))

  def getTweetsByKeyword: List[String] => TweetSet =
    getTweetsByKeywordsFromTweetSet(
      TweetReader.allTweets
    )(_)

  lazy val googleTweets: TweetSet =
    getTweetsByKeyword(google)
  lazy val appleTweets: TweetSet =
    getTweetsByKeyword(apple)
